...
function waitForCM(callback) {
    if (window.cm && window.cm.EditorView) {
        callback();
    } else {
        setTimeout(() => waitForCM(callback), 100);
    }
}

waitForCM(() => {
    const { EditorView, basicSetup, keymap } = window.cm;
    const { EditorState } = window.cms;
    const { defaultKeymap, history, historyKeymap, indentWithTab } = window.cmc;
    const { syntaxHighlighting, defaultHighlightStyle, bracketMatching, indentOnInput } = window.cml;
    const { autocompletion, completionKeymap, closeBrackets, closeBracketsKeymap } = window.cma;
    const { javascript } = window.cmjs;
    const { python } = window.cmp;
    const { cpp } = window.cmcpp;
    const { html } = window.cmhtml;
    ...
    const startPath = 'cdvfile://localhost/persistent/';
